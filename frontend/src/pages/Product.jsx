              ...
              <div className='flex sm:flex-col overflow-x-auto sm:overflow-y-scroll justify-between gap-3 w-full sm:justify-normal sm:w-[18.7%]'>
                 {
                  productData.image.map((item,index)=>(
                    <img key={index} onClick={()=>setImage(item)} className={`w-[24%] sm:w-full cursor-pointer sm:mb-3 flex-shrink-0 `} src={item} alt={productData.name} />
                  ))
                 }
                      ...
                      <div className='flex gap-2'>
                          {productData.sizes.map((item, index) => (
                            <button 
                              ...
                              className={`border py-2 px-4 bg-slate-200 ${item === size ? 'border-orange-500' : ''}`} 
                              key={index}
                            >
                              {item}
                            </button>
