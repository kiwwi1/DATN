...
\begin{equation}
    \text{UpdateFilter} = \{ \text{\_id}: \text{productId},\ \texttt{isActive}: \text{true},\ \text{stock}: \{ \$gte: \text{requestedQuantity} \} \}
\end{equation}
\begin{equation}
...
\end{equation}
Trong đó $\Delta t$ là số ngày kể từ lần tương tác gần nhất (\texttt{lastInteraction}) đến thời điểm tính toán: $\Delta t = (t_{current} - t_{lastInteraction}) / (24 \times 3600 \times 1000)$. Hệ số $\lambda = \frac{1}{30}$ tương ứng với chu kỳ bán rã 30 ngày (sau 30 ngày không có tương tác mới, điểm quan tâm của người dùng đối với sản phẩm giảm đi một nửa).
...
\begin{itemize}
    ...
    \item \textbf{Giải quyết triệt để lỗi đề xuất cũ kỹ}: Nhờ hệ số suy hao thời gian $\lambda = \frac{1}{30} \approx 0.033$ (chu kỳ bán rã 30 ngày), các sản phẩm người dùng tìm kiếm từ nhiều tuần trước sẽ nhanh chóng nhường vị trí ưu tiên cho các mối quan tâm mới nhất của họ.
