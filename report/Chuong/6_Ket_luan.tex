...
\begin{itemize}
    ...
    \item \textbf{Đảm bảo tính toàn vẹn tồn kho dưới tải cao}: Bằng việc thiết kế và triển khai cơ chế giữ kho nguyên tử (Atomic Stock Reservation) ở tầng cơ sở dữ liệu MongoDB kết hợp khóa chống trùng lặp giao dịch (Idempotency Key), hệ thống kiểm soát rủi ro bán quá mức (oversell) và hạn chế tạo đơn hàng trùng khi người dùng gửi nhiều yêu cầu đồng thời.
    \item \textbf{Nâng cao trải nghiệm cá nhân hóa}: Tích hợp công cụ gợi ý sản phẩm lai (Hybrid Recommendation Engine) chấm điểm tương tác động theo chín loại hành vi người dùng (từ lượt xem, tìm kiếm cho đến thêm giỏ hàng và mua hàng) kết hợp hệ số suy hao thời gian (Time Decay), giúp đưa ra những đề xuất sản phẩm chính xác, phù hợp nhất với hành vi mua sắm thay đổi theo thời gian của từng khách hàng.
    ...
    \item \textbf{Thông báo và tương tác thời gian thực}: Thiết lập kênh Server-Sent Events (SSE) cho thông báo và cập nhật trạng thái đơn hàng một chiều, đồng thời sử dụng Socket.IO cho trò chuyện hai chiều giữa người mua và người bán.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Hiệu năng tính toán gợi ý khi dữ liệu tăng lớn}: Bộ gợi ý sản phẩm lai hiện tại kết hợp truy vấn MongoDB với Redis cache ngắn hạn cho từng người dùng. Cách tiếp cận này phù hợp với quy mô thử nghiệm, nhưng khi ma trận tương tác tăng lên hàng triệu bản ghi, việc tính toán lại sau khi cache hết hạn vẫn có thể gây trễ (latency) cho luồng tải trang.
...
\begin{itemize}
    \item \textbf{Tiền tính toán và cache dài hạn (Pre-computing \& Extended Caching)}: Chuyển dịch tiến trình chấm điểm tương tác và tính toán danh sách gợi ý sản phẩm thành các tác vụ chạy ngầm bất đồng bộ định kỳ (offline computing). Danh sách gợi ý sau khi tính toán sẽ được lưu trữ bằng cơ chế cache dài hạn hoặc vật hóa vào cơ sở dữ liệu, trong khi Redis tiếp tục đóng vai trò cache tốc độ cao ở lớp phục vụ.
...
\section*{Kết chương}
Chương 6 đã tổng kết toàn bộ quá trình thực hiện đồ án, từ các kết quả đã đạt được trong xây dựng hệ thống \textbf{Lumière} đến những hạn chế còn tồn tại về hiệu năng khuyến nghị, khả năng mở rộng kênh thông báo thời gian thực và mức độ tích hợp dịch vụ logistics. Trên cơ sở đó, chương đã đề xuất các hướng phát triển tiếp theo như tiền tính toán dữ liệu khuyến nghị, bổ sung hàng đợi tin nhắn, tập trung hóa SSE bằng Redis Pub/Sub, nâng cấp mô hình học máy và kết nối với các dịch vụ vận chuyển thực tế. Những định hướng này mở ra khả năng hoàn thiện hệ thống theo hướng ổn định, mở rộng và phù hợp hơn với môi trường vận hành thương mại điện tử thực tế.
\end{document}
